---
title: "IsabelaZeberio_100386498_TMFinalAssignment"
...
In order to convert the scripts into variables we have to read the webpage using `read_html()` and extract the text from. The function `xml_child()` is used to obtain the nodes of the webpage. The instructions are the same for all 3 scripts, it is needed to obtain the character vector with the text of the scripts.
...
-   The function `xml_find_all()` allow us to extract the specific nodes containing the screeplay text.
...
In Rushmore we see that the two most frequent words are Max and Blume, character names. The next two are a little bit confusing if you have not seen the movies since the other really important character of this movie is Miss Rosemary Cross.

After the names of the characters, the most repeated words that begin to give us some clues about the overall tone of this film are dirk, pause, nods, smiles, silence, hands, starts, eyes, and school. Many of the words we see describe the actions ore movement the actors have to follow. However, we can perceive in this first reading a contemplative, intimate or introspective tone with moments of silence, nods, and eye contact indicating characters are reflecting on their thoughts and feelings. The world school is also a great indicator of where this story takes place.
...
There is a column named "Rushmore" with proportion of the movie because it is our reference. We are comparing the words used in Moonrise Kingdom and Fantastic Mr. Fox with the ones used in Rushmore.
...
Using sentiment analysis on Wes Anderson's scripts could reveal information about the emotional tone of his films. Wes Anderson's films are noted for their quirky and enchanting style, and sentiment analysis could contribute to uncovering the underlying emotional themes that run throughout his films.
...
This plot is very interesting because despite Anderson's aesthetics, the feelings we see in his films are

predominantly negative (I would rather say nostalgic):
...
Now we can observe how different narrative plot can be just depending on the sentiment lexicon we use.
...
A popular technique for visualizing the most frequently occurring words in a piece of text is wordcloud. It is a quick and easy technique to summarize the main themes and topics in a text, allowing us to rapidly identify the most significant ideas and concepts.
...
The three scripts follow a similar distribution: positive skewed. Most words have very low frequencies (on the left) and a few words have very high frequencies (on the right). Rushmore seems to have the biggest amount of words in the last category of the right.

### Zipf's Law

Zipf's Law is an empirical law that asserts that the frequency of each word in a large sample of texts is inversely related to its rank in the frequency table. To test it we add column for the rank to the dataframe, ranking the words in descending order by their frequency in the movie.
...
We use this coefficients to draw a line in the plot to observe the deviation from the standard use of language. This is a great way to visualize the Wes Anderson's deviation, specially in the first section (upper left part).
...
bigram_tf_idf %>%
  ...
  facet_wrap(~movie, ncol = 2, scales = "free") +
  labs(x = "tf-idf", y = NULL) +
  #The French Dispatch is Wes Anderson most recent movie
